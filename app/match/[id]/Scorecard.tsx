...
interface Innings {
  id: number; innings_number: number;
  batting_team: string; bowling_team: string;
  total_runs: number; total_wickets: number; overs: number;
}
interface BallEvent {
  id: number; innings_id: number;
  over_number: number; ball_number: number;
  batsman_id: number | null; bowler_id: number | null;
  runs: number; extra_runs: number; extra_type: string | null;
  is_wicket: boolean; wicket_type: string | null;
  dismissed_batsman_id?: number | null; new_batsman_id?: number | null;
}
...
const rotatesStrike = (b: BallEvent): boolean => {
  ...
  if (b.extra_type === "bye" || b.extra_type === "leg bye")
    return (b.runs + b.extra_runs) % 2 === 1;
  return b.runs % 2 === 1;
};

// ─── computeStats: works for ANY innings ─────────────────────────────────────
function computeStats(
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    const toBowl = b.extra_type === "bye" || b.extra_type === "leg bye" ? b.extra_runs
      : b.extra_type === "no ball" ? 1 + b.runs
      : b.extra_type === "wide"    ? 1 + b.runs
      : b.runs;
    ...
    // ── batsman (always, even on dismissal ball) ──
    if (b.batsman_id) {
      const s = batStats.get(b.batsman_id) ?? { runs: 0, balls: 0, fours: 0, sixes: 0 };
      s.runs += toBat;
      if (legal) { s.balls++; if (b.runs === 4) s.fours++; if (b.runs === 6) s.sixes++; }
    ...
    if (b.is_wicket && b.dismissed_batsman_id) {
      ...
      fow.push({
        over: toOvers(legalBalls), runs, wicket: wickets,
    ...
    // over-end: rotate + clear bowler (only when more balls follow)
  ...
  for (const [bid, om] of bowlerOverRuns.entries()) {
    ...
    const s = bowlStats.get(bid); if (s) s.maidens = m;
  }
  ...
  if (target && innings.innings_number === 2) {
    const needed = target - runs, left = totalOvers - overs;
  ...
  return {
    runs, wickets, legalBalls, crr, rrr,
    striker, nonStriker, bowler,
    batStats, bowlStats, fow, last6,
    yetToBat, partnership: { runs: pRuns, balls: pBalls },
...
export default function Scorecard({
  matchId, teamA, teamALogo, teamB, teamBLogo, totalOvers, matchStatus, isOwner,
}: {
  matchId: number; teamA: string; teamALogo?: string;
  teamB: string; teamBLogo?: string; totalOvers: number;
  matchStatus: string; isOwner: boolean;
}) {
  ...
  // On-field local state (owner only, survives across balls)
  ...
  const justStartedRef  = useRef(false);  // Prevents auto‑complete for 2 seconds after new innings

  // ── derived ──
  const target = allInnings.length >= 2 ? allInnings[0].total_runs + 1 : null;

  const displayedInnings = useMemo(
    () => viewInningsId != null ? allInnings.find(i => i.id === viewInningsId) ?? null : currentInnings,
    [viewInningsId, allInnings, currentInnings]
  );
  const displayedBalls = viewInningsId === currentInnings?.id ? ballEvents : viewBalls;

  const displayedStats = useMemo(
    () => computeStats(displayedInnings, displayedBalls, players, target, totalOvers),
    [displayedInnings, displayedBalls, players, target, totalOvers]
  );
  const currentStats = useMemo(
    () => computeStats(currentInnings, ballEvents, players, target, totalOvers),
    [currentInnings, ballEvents, players, target, totalOvers]
  );

  // ── innings completion (strict: needs at least one legal ball) ──
  const inningsComplete = useCallback((): boolean => {
    if (!currentInnings || !currentStats) return false;
    if (ballEvents.length === 0) return false;
    if (currentStats.legalBalls === 0) return false; // extra safety
  ...
  const getBatsmen = useCallback((excludeIds: (number | null)[] = []) => {
  ...
  const matchWinner = useMemo(() => {
    ...
    if (s.total_runs > f.total_runs) {
      ...
      const margin = totalBatsmen - s.total_wickets;  // wickets in hand (correct)
  ...
  // ─── refreshData ─────────────────────────────────────────────────────────
  const refreshData = useCallback(async (spinner = true, forceViewLatest = false) => {
  ...
  useEffect(() => {
    (async () => { setInitialLoading(true); await refreshData(false); setInitialLoading(false); })();
  }, [refreshData]);

  useEffect(() => {
    if (viewInningsId && viewInningsId !== currentInnings?.id) {
      fetch(`/api/score?inningsId=${viewInningsId}`).then(r => r.json()).then(setViewBalls);
    } else {
      setViewBalls([]);
    }
  ...
  // Pusher
  useEffect(() => {
    let pusher: any = null, ch: any = null, alive = true;
    (async () => {
    ...
    return () => {
      alive = false;
      try { ch?.unbind_all(); pusher?.unsubscribe(`match-${matchId}`); pusher?.disconnect(); } catch {}
    };
  }, [matchId, refreshData]);

  // ── auto-complete with guard for just started innings ──
  useEffect(() => {
    if (!isOwner || matchStatus === "completed" || autoCompleteRef.current) return;
    if (justStartedRef.current) return; // Skip for 2 seconds after new innings
  ...
  // reset local state on innings change
  useEffect(() => {
    if (!currentInnings) return;
    if (inningsIdRef.current !== currentInnings.id) {
      inningsIdRef.current = currentInnings.id;
      setLocalStriker(null); setLocalNonStriker(null); setLocalBowler(null);
      autoCompleteRef.current = false; autoModalRef.current = false;
  ...
  useEffect(() => {
    ...
    if (noBalls) {
      if      (!localStriker)    { setShowStriker(true); return; }
      else if (!localNonStriker) { setShowNonStr(true);  return; }
      else if (!localBowler)     { setShowBowler(true);  return; }
    ...
    isOwner, currentInnings?.id, isViewingCurrent, ballEvents.length,
    currentStats?.isOverComplete, currentStats?.bowler,
    localStriker, localNonStriker, localBowler, inningsComplete,
  ]);

  // effective positions shown in UI
  ...
  // ── optimistic local update after each ball ──
  const applyLocal = (
    runs: number, extra: string | null, isWicket: boolean,
    dis: number | null, newB: number | null, prevLegal: number
  ) => {
    let s = localStriker, ns = localNonStriker;
  ...
  const recordBall = async (
    runs: number, extra: string | null = null,
    isWicket = false, wType: string | null = null,
    dis?: number, newBat?: number
  ) => {
    if (!isOwner || !currentInnings || !isViewingCurrent || inningsComplete()) return;
    if (!localStriker) { setShowStriker(true); return; }
    if (!localBowler)  { setShowBowler(true);  return; }
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        runs, is_wicket: isWicket, wicket_type: wType ?? null,
        extra_type:  extra ?? null,
        extra_runs:  extra === "wide" || extra === "no ball" ? 1 : 0,
      };
      if (isWicket && dis) { payload.dismissed_batsman_id = dis; payload.new_batsman_id = newBat ?? null; }
      ...
      if (res.ok) {
        ...
        if (total > 0) { setRunFlash({ on: true, val: total }); setTimeout(() => setRunFlash({ on: false, val: 0 }), 1100); }
        applyLocal(runs, extra, isWicket, dis ?? null, newBat ?? null, legal);
        await refreshData(true);
        setShowWicket(false); setWkType(null); setDismissedId(null); setNewBatId(null);
      }
    } catch (e) { console.error(e); }
    setSubmitting(false);
  };
  ...
  const undoLast = async () => {
    ...
    setUndoing(true);
    try {
      await fetch(`/api/score/last-ball?inningsId=${currentInnings.id}`, { method: "DELETE" });
      setLocalBowler(null); autoModalRef.current = false;
      await refreshData(true);
    } catch (e) { console.error(e); }
    setUndoing(false);
  };
  ...
  const resetInn = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setLocalStriker(null); setLocalNonStriker(null); setLocalBowler(null);
        inningsIdRef.current = null; autoCompleteRef.current = false; autoModalRef.current = false;
        justStartedRef.current = false;
        await refreshData(true);
      } else { alert((await res.json()).error ?? "Reset failed"); }
  ...
  const startSecondInnings = async () => {
    ...
    try {
      ...
      setLocalStriker(null); setLocalNonStriker(null); setLocalBowler(null);
      inningsIdRef.current = null; autoCompleteRef.current = false; autoModalRef.current = false;
      justStartedRef.current = true;   // Disable auto‑complete for 2 seconds
      setTimeout(() => { justStartedRef.current = false; }, 2000);
  ...
  if (initialLoading) return (
    <div className="flex flex-col items-center justify-center py-24 gap-3">
      ...
      <span className="text-[#a8dadc]/50 text-[10px] font-black uppercase tracking-widest">Loading…</span>
    </div>
  );
  if (!currentInnings) return (
    <div className="flex flex-col items-center justify-center py-24 gap-3">
  ...
  displayedBalls.forEach(b => { if (!oversMap.has(b.over_number)) oversMap.set(b.over_number, []); oversMap.get(b.over_number)!.push(b); });
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 space-y-4 sm:space-y-5 pb-36 select-none">
      ...
      <AnimatePresence>
        {runFlash.on && (
          <motion.div key={runFlash.val + "-" + Date.now()} initial={{ opacity: 0, scale: 0.4 }} animate={{ opacity: 1, scale: 1.3 }} exit={{ opacity: 0, scale: 0.6, y: -40 }} transition={{ duration: 0.45 }}
            className="fixed inset-0 flex items-center justify-center z-[200] pointer-events-none">
            <span className={`text-7xl sm:text-9xl font-black drop-shadow-2xl ${runFlash.val >= 6 ? "text-purple-400" : runFlash.val >= 4 ? "text-green-400" : "text-white"}`}>
              {runFlash.val}
            </span>
          </motion.div>
        )}
      ...
      {allInnings.length > 1 && (
        <div className="flex gap-2">
          {allInnings.map(inn => (
            <button key={inn.id} onClick={() => setViewInningsId(inn.id)}
              className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border transition-all ${viewInningsId === inn.id ? "bg-[#e63946] border-[#e63946] text-white" : "bg-[#1d3557] border-[#457b9d]/40 text-[#a8dadc] hover:border-[#e63946]/40"}`}>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden border border-[#457b9d]/50 bg-[#0f1e32] flex items-center justify-center shrink-0">
                {dispLogo ? <img src={dispLogo} alt={dispName} className="w-full h-full object-cover" /> : <span className="text-2xl font-black text-[#457b9d]/40">🏏</span>}
              </div>
              <div>
                <div className="text-[#a8dadc]/70 text-[9px] font-black uppercase tracking-widest mb-0.5">{dispName}</div>
                <div className="flex items-baseline gap-1.5">
                  <motion.span animate={runFlash.on ? { scale: [1, 1.25, 1], color: ["#fff", "#e63946", "#fff"] } : { scale: 1 }} transition={{ duration: 0.4 }}
                    className="text-5xl sm:text-6xl font-black tracking-tight leading-none text-white">{st?.runs ?? 0}</motion.span>
                  <span className="text-2xl sm:text-3xl font-black text-[#e63946] leading-none">/{st?.wickets ?? 0}</span>
                </div>
                <div className="flex items-center gap-2 mt-1.5">
                  <span className="text-xs font-bold text-[#a8dadc]/50">{toOvers(st?.legalBalls ?? 0)} / {totalOvers} ov</span>
                  {st?.isOverComplete && !inningsComplete() && isViewingCurrent && (
                    <span className="px-1.5 py-0.5 bg-amber-500/20 border border-amber-500/40 text-amber-400 text-[9px] font-black uppercase rounded animate-pulse">Over End</span>
                  )}
                </div>
              </div>
            </div>
              ...
              {isOwner && !isMatchDone && isViewingCurrent && (
                <div className="flex flex-wrap gap-1.5">
                  {inningsComplete() && allInnings.length === 1 && (
                    <button onClick={startSecondInnings} disabled={submitting}
                      className="px-3 py-1.5 bg-[#e63946] text-white font-black text-[9px] uppercase tracking-wider rounded-lg disabled:opacity-50">
                      2nd Innings →
                    </button>
                  )}
                  <button onClick={undoLast} disabled={undoing || ballEvents.length === 0}
                    className="px-3 py-1.5 bg-[#457b9d]/80 text-white font-black text-[9px] uppercase tracking-wider rounded-lg disabled:opacity-40 hover:bg-[#457b9d]">↩ Undo</button>
                  <button onClick={resetInn} disabled={resetting}
                    className="px-3 py-1.5 bg-red-900/60 border border-red-700/50 text-red-400 font-black text-[9px] uppercase tracking-wider rounded-lg disabled:opacity-50">Reset</button>
                  <button onClick={finishMatch}
                    className="px-3 py-1.5 bg-[#1d3557] border border-[#e63946]/40 text-[#e63946] font-black text-[9px] uppercase tracking-wider rounded-lg hover:border-[#e63946]">Finish</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex gap-1">
                ...
                {st?.last6.map((b, i) => (
                  <span key={i} className={`w-7 h-7 flex items-center justify-center rounded-lg text-xs font-black ${b === "W" ? "bg-red-500/25 text-red-400 border border-red-500/30" : b === "6" ? "bg-purple-500/25 text-purple-300 border border-purple-500/30" : b === "4" ? "bg-green-500/25 text-green-400 border border-green-500/30" : "bg-[#457b9d]/20 text-[#a8dadc] border border-[#457b9d]/15"}`}>{b}</span>
                ))}
              </div>
            </div>
            {isViewingCurrent && (efStriker || efBowler) && (
              <div className="hidden sm:flex items-center gap-3 text-[9px] font-black uppercase tracking-wider">
                {efStriker    && <span className="text-[#f1faee]"><span className="text-[#e63946]">★ </span>{players.find(p => p.id === efStriker)?.name}</span>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-[#1d3557]/60 rounded-2xl border border-[#457b9d]/25 overflow-hidden">
          <div className="px-4 py-3 border-b border-[#457b9d]/20"><h3 className="text-[10px] font-black uppercase tracking-widest text-[#e63946]">🏏 Batting</h3></div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="text-[#a8dadc]/50 text-[9px] font-black uppercase border-b border-[#457b9d]/15">
                <th className="text-left px-4 py-2.5">Batter</th>
                <th className="text-center px-2 py-2.5">R</th><th className="text-center px-2 py-2.5">B</th>
                <th className="text-center px-2 py-2.5">4s</th><th className="text-center px-2 py-2.5">6s</th>
                <th className="text-center px-2 py-2.5">SR</th>
              </tr></thead>
              <tbody>
                {batRows.filter(r => r.shown).map(r => (
                  <tr key={r.p.id} className={`border-b border-[#457b9d]/10 ${r.isStr ? "bg-[#e63946]/8" : r.isNS ? "bg-[#457b9d]/8" : ""}`}>
                    <td className="px-4 py-2.5 font-bold text-[#f1faee] whitespace-nowrap">
                      <div className="flex items-center gap-1.5">
                        ...
                        {!r.isStr && !r.isNS && <span className="w-1.5 h-1.5" />}
                ...
                {batRows.filter(r => r.shown).length === 0 && (
                  <tr><td colSpan={6} className="text-center py-6 text-[#a8dadc]/30 text-[10px] font-bold uppercase tracking-widest">Innings not started</td></tr>
                )}
              </tbody>
            </table>
        ...
        <div className="bg-[#1d3557]/60 rounded-2xl border border-[#457b9d]/25 overflow-hidden">
          <div className="px-4 py-3 border-b border-[#457b9d]/20"><h3 className="text-[10px] font-black uppercase tracking-widest text-[#e63946]">⚡ Bowling</h3></div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="text-[#a8dadc]/50 text-[9px] font-black uppercase border-b border-[#457b9d]/15">
                <th className="text-left px-4 py-2.5">Bowler</th>
                <th className="text-center px-2 py-2.5">O</th><th className="text-center px-2 py-2.5">M</th>
                <th className="text-center px-2 py-2.5">R</th><th className="text-center px-2 py-2.5">W</th>
                <th className="text-center px-2 py-2.5">Econ</th>
              </tr></thead>
              <tbody>
                {bowlRows.filter(r => r.shown).map(r => (
                  <tr key={r.p.id} className={`border-b border-[#457b9d]/10 ${r.cur ? "bg-[#4ade80]/5" : ""}`}>
                    <td className="px-4 py-2.5 font-bold text-[#f1faee] whitespace-nowrap">
                      <div className="flex items-center gap-1.5">
                        ...
                        {!r.cur && <span className="w-1.5 h-1.5" />}
                        {r.p.name}
                      </div>
                    </td>
                ...
                {bowlRows.filter(r => r.shown).length === 0 && (
                  <tr><td colSpan={6} className="text-center py-6 text-[#a8dadc]/30 text-[10px] font-bold uppercase tracking-widest">No overs bowled</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[#1d3557]/40 rounded-xl border border-[#457b9d]/20 p-4">
          ...
          <div className="text-2xl font-black text-white">{st?.partnership.runs ?? 0} <span className="text-sm text-[#a8dadc]/50 font-bold">({st?.partnership.balls ?? 0}b)</span></div>
          <div className="text-[10px] text-[#a8dadc]/60 mt-1 truncate">
            {isViewingCurrent ? `${players.find(p => p.id === efStriker)?.name ?? "—"} & ${players.find(p => p.id === efNonStriker)?.name ?? "—"}` : "—"}
          </div>
        </div>
        <div className="bg-[#1d3557]/40 rounded-xl border border-[#457b9d]/20 p-4">
          ...
          {(st?.fow.length ?? 0) === 0
            ...
            : <div className="space-y-0.5 max-h-24 overflow-y-auto">{st!.fow.map((f, i) => (
                <div key={i} className="flex justify-between text-[10px]">
                  <span className="text-[#a8dadc]/70">{f.wicket}-{f.runs} <span className="text-[#a8dadc]/40">({f.batsman})</span></span>
        ...
        <div className="bg-[#1d3557]/40 rounded-xl border border-[#457b9d]/20 p-4">
          ...
          {(st?.yetToBat.length ?? 0) === 0
            ...
            : <div className="flex flex-wrap gap-1">{st!.yetToBat.map(p => (
                <span key={p.id} className="text-[9px] bg-[#1d3557] border border-[#457b9d]/30 text-[#a8dadc]/70 px-2 py-0.5 rounded-full">{p.name}</span>
              ))}</div>}
        </div>
      </div>
      ...
      <div className="bg-[#1d3557]/40 rounded-2xl border border-[#457b9d]/20 p-4 sm:p-5">
        ...
        {oversMap.size === 0
          ...
          : <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {Array.from(oversMap.entries()).sort((a, b) => a[0] - b[0]).map(([ov, bs]) => {
              ...
              return (
                <div key={ov} className="bg-[#0f1e32]/60 rounded-xl border border-[#457b9d]/20 overflow-hidden">
                  <div className="flex items-center justify-between px-3 py-2 border-b border-[#457b9d]/15">
                    ...
                    <span className="text-[9px] font-black text-[#a8dadc]/50">{oR}R{oW > 0 ? ` ${oW}W` : ""}</span>
                  </div>
                  <div className="p-2 space-y-0.5">
                    {bs.map((b, idx) => {
                      const out = b.is_wicket ? `⚡ ${b.wicket_type ?? "Wicket"}`
                        : b.extra_type ? `${b.extra_type.toUpperCase()}${b.extra_runs ? ` +${b.extra_runs}` : ""}`
                        : b.runs === 0 ? "Dot" : `${b.runs} run${b.runs !== 1 ? "s" : ""}`;
                      return (
                        <div key={idx} className={`flex justify-between items-center rounded px-2 py-1 text-[9px] ${b.is_wicket ? "bg-red-500/10 text-red-400" : "text-[#a8dadc]/70"}`}>
                          ...
                          <span className="text-[#a8dadc]/40">{players.find(p => p.id === b.batsman_id)?.name ?? "?"} · {players.find(p => p.id === b.bowler_id)?.name ?? "?"}</span>
                        </div>
                      );
                    })}
                  </div>
      ...
      {scoringAllowed && (
        <motion.div initial={{ y: 120, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: "spring", stiffness: 300, damping: 28 }}
          ...
          <div className="flex items-center justify-center gap-1.5 flex-wrap max-w-2xl mx-auto">
            {[0,1,2,3,4,6].map(r => (
              <button key={r} onClick={() => recordBall(r)} disabled={submitting}
                className={`w-11 h-11 sm:w-12 sm:h-12 rounded-xl font-black text-lg sm:text-xl transition-all active:scale-90 disabled:opacity-40 ${r === 4 ? "bg-green-600/80 hover:bg-green-600 text-white border border-green-500/40" : r === 6 ? "bg-purple-600/80 hover:bg-purple-600 text-white border border-purple-500/40" : "bg-[#1d3557] hover:bg-[#457b9d] text-white border border-[#457b9d]/40"}`}>{r}</button>
            ))}
            <div className="w-px h-10 bg-[#457b9d]/20" />
            <button onClick={() => recordBall(0, "wide")}    disabled={submitting} className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl font-black text-[10px] bg-amber-900/60 hover:bg-amber-800/80 text-amber-400  border border-amber-700/40  active:scale-90 disabled:opacity-40">WD</button>
            <button onClick={() => recordBall(0, "no ball")} disabled={submitting} className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl font-black text-[10px] bg-orange-900/60 hover:bg-orange-800/80 text-orange-400 border border-orange-700/40 active:scale-90 disabled:opacity-40">NB</button>
            <button onClick={() => recordBall(0, "bye")}     disabled={submitting} className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl font-black text-[9px]  bg-[#1d3557] hover:bg-[#457b9d]/60 text-[#a8dadc]/70 border border-[#457b9d]/30  active:scale-90 disabled:opacity-40">BYE</button>
            <div className="w-px h-10 bg-[#457b9d]/20" />
            <button onClick={() => setShowWicket(true)} disabled={submitting} className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl font-black text-sm bg-[#e63946] hover:bg-[#c1121f] text-white border border-[#e63946]/60 active:scale-90 disabled:opacity-40">W</button>
            <div className="w-px h-10 bg-[#457b9d]/20" />
            <button onClick={() => setShowBowler(true)}  className="h-11 sm:h-12 px-3 rounded-xl font-black text-[9px] bg-[#0f1e32] hover:bg-[#1d3557] text-[#4ade80]/70 border border-[#4ade80]/20 uppercase tracking-wider">Bowler</button>
            <button onClick={() => setShowStriker(true)} className="h-11 sm:h-12 px-3 rounded-xl font-black text-[9px] bg-[#0f1e32] hover:bg-[#1d3557] text-[#e63946]/70 border border-[#e63946]/20 uppercase tracking-wider">Set</button>
          </div>
        </motion.div>
      )}
      ...
      <AnimatePresence>
        {showBowler && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/75 flex items-end sm:items-center justify-center p-4 z-[70]" onClick={() => setShowBowler(false)}>
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} exit={{ y: 50 }} onClick={e => e.stopPropagation()} className="bg-[#1d3557] rounded-2xl border border-[#457b9d]/40 w-full max-w-sm overflow-hidden">
              ...
              <div className="p-3 space-y-1.5 max-h-72 overflow-y-auto">
                {getBowlers().map(p => (
                  <button key={p.id} onClick={() => { setLocalBowler(p.id); setShowBowler(false); autoModalRef.current = false; }} className="w-full p-3 bg-[#0f1e32]/60 hover:bg-[#457b9d]/30 rounded-xl text-left transition-all">
                    <span className="text-sm font-bold text-[#f1faee]">{p.name}</span>
                  </button>
                ))}
                {getBowlers().length === 0 && <div className="text-center py-4 text-[#a8dadc]/40 text-xs">No bowlers available</div>}
              </div>
              <div className="p-3 border-t border-[#457b9d]/20"><button onClick={() => setShowBowler(false)} className="w-full py-2.5 rounded-xl border border-[#457b9d]/30 text-[#a8dadc]/60 text-xs font-black uppercase">Cancel</button></div>
            </motion.div>
          </motion.div>
        )}

        {showStriker && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/75 flex items-end sm:items-center justify-center p-4 z-[70]" onClick={() => setShowStriker(false)}>
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} exit={{ y: 50 }} onClick={e => e.stopPropagation()} className="bg-[#1d3557] rounded-2xl border border-[#457b9d]/40 w-full max-w-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-[#457b9d]/20">
              ...
              <div className="p-3 space-y-1.5 max-h-72 overflow-y-auto">
                {getBatsmen([localNonStriker]).map(p => (
                  <button key={p.id} onClick={() => { setLocalStriker(p.id); setShowStriker(false); if (!localNonStriker) setShowNonStr(true); }}
              ...
              <div className="p-3 border-t border-[#457b9d]/20"><button onClick={() => setShowStriker(false)} className="w-full py-2.5 rounded-xl border border-[#457b9d]/30 text-[#a8dadc]/60 text-xs font-black uppercase">Cancel</button></div>
            </motion.div>
          </motion.div>
        )}

        {showNonStr && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/75 flex items-end sm:items-center justify-center p-4 z-[70]" onClick={() => setShowNonStr(false)}>
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} exit={{ y: 50 }} onClick={e => e.stopPropagation()} className="bg-[#1d3557] rounded-2xl border border-[#457b9d]/40 w-full max-w-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-[#457b9d]/20">
              ...
              <div className="p-3 space-y-1.5 max-h-72 overflow-y-auto">
                {getBatsmen([localStriker]).map(p => (
                  <button key={p.id} onClick={() => { setLocalNonStriker(p.id); setShowNonStr(false); if (!localBowler) setShowBowler(true); }}
              ...
              <div className="p-3 border-t border-[#457b9d]/20"><button onClick={() => setShowNonStr(false)} className="w-full py-2.5 rounded-xl border border-[#457b9d]/30 text-[#a8dadc]/60 text-xs font-black uppercase">Cancel</button></div>
            </motion.div>
          </motion.div>
        )}

        {showWicket && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 flex items-end sm:items-center justify-center p-4 z-[70]">
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} exit={{ y: 50 }} className="bg-[#1d3557] rounded-2xl border border-[#e63946]/30 w-full max-w-sm overflow-hidden">
              ...
              <div className="p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {[localStriker, localNonStriker].filter(Boolean).map(id => (
                      <button key={id} onClick={() => setDismissedId(id!)}
                        className={`p-2.5 rounded-xl text-sm font-bold transition-all ${dismissedId === id ? "bg-[#e63946] text-white" : "bg-[#0f1e32]/60 text-[#f1faee] border border-[#457b9d]/20 hover:bg-[#e63946]/20"}`}>
                        {players.find(p => p.id === id)?.name}{id === localStriker ? " ★" : ""}
                      </button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {WKTS.map(w => (
                      <button key={w} onClick={() => setWkType(w)}
                        className={`p-2 rounded-lg text-[10px] font-black uppercase transition-all ${wkType === w ? "bg-[#e63946] text-white" : "bg-[#0f1e32]/60 text-[#a8dadc]/70 border border-[#457b9d]/20 hover:bg-[#e63946]/15"}`}>{w}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  {(st?.yetToBat.length ?? 0) > 0
                    ? <div className="space-y-1 max-h-28 overflow-y-auto">
                        {st!.yetToBat.map(p => (
                          <button key={p.id} onClick={() => setNewBatId(p.id)}
                            className={`w-full p-2.5 rounded-xl text-sm font-bold text-left transition-all ${newBatId === p.id ? "bg-[#457b9d] text-white" : "bg-[#0f1e32]/60 text-[#f1faee] border border-[#457b9d]/20 hover:bg-[#457b9d]/30"}`}>
                            {p.name}
                          </button>
                        ))}
                      </div>
              ...
              <div className="p-4 pt-0 space-y-2">
                <button onClick={() => {
                  if (!dismissedId || !wkType) { alert("Select dismissed batsman and wicket type"); return; }
                  if ((st?.yetToBat.length ?? 0) > 0 && !newBatId) { alert("Select the new batsman"); return; }
                ...
                <button onClick={() => { setShowWicket(false); setWkType(null); setDismissedId(null); setNewBatId(null); }}
                  className="w-full py-2.5 rounded-xl border border-[#457b9d]/30 text-[#a8dadc]/60 text-xs font-black uppercase">Cancel</button>
              </div>
            </motion.div>
          </motion.div>
        )}
