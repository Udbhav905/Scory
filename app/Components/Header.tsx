...
function formatScore(innings: InningsRow[]): string | null {
  ...
  const last = innings.reduce((prev, curr) =>
...
function NavLink({
  ...
  return (
    <Link
      href={href}
      className={`relative text-[11px] font-black uppercase tracking-[0.2em] transition-colors duration-200 px-1 py-0.5 ${
        active ? "text-[#B5E18B]" : "text-[#8090A4] hover:text-[#E0E8F0]"
      }`}
    >
      {label}
      {active && (
        <span className="absolute -bottom-px left-0 right-0 h-px bg-[#B5E18B] rounded-full" />
      )}
    </Link>
  );
}
...
function TickerItem({ match }: { match: TickerMatch }) {
  return (
    <Link
      href={`/match/${match.id}`}
      className="flex items-center gap-2 px-6 font-['Barlow_Condensed',sans-serif] text-[11px] font-semibold tracking-wide text-[#8090A4] uppercase border-r border-white/[0.05] hover:text-[#B5E18B] transition-colors whitespace-nowrap shrink-0"
    >
      ...
      {match.overs && (
        <span className="text-[#8090A4]">({match.overs})</span>
      )}
      {match.live && (
...
export default function Header() {
      ...
      try {
        ...
        const settled = await Promise.allSettled(
          matches.map(async (match) => {
            ...
            const isLive =
              match.status === "live" ||
              match.status === "innings_1_complete" ||
              match.status === "innings_2_live";
            return {
              id: match.id,
        ...
        const valid = settled
          .filter(
            (r): r is PromiseFulfilledResult<TickerMatch | null> =>
              r.status === "fulfilled" && r.value !== null
          )
          .map((r) => r.value as TickerMatch);
  ...
  const displayedItems: TickerMatch[] =
      ...
      <header className="sticky top-0 z-50 font-sans">
        ...
        <div className="relative bg-[#080a0f]/95 backdrop-blur-2xl border-b border-white/[0.05] shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-4">
            ...
            <Link href="/" className="group flex items-center gap-3 shrink-0">
              ...
              <div className="flex flex-col leading-none">
                ...
                <div className="font-['Barlow_Condensed',sans-serif] font-semibold text-[9px] tracking-[0.35em] text-[#8090A4] uppercase">
                  Live Cricket
                </div>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {status === "authenticated" ? (
                <div className="relative" ref={dropdownRef}>
                  <button
                    ...
                    className="flex items-center gap-2 px-2 py-1 rounded-xl hover:bg-white/5 transition-colors duration-200"
                    aria-expanded={dropdownOpen}
                    aria-haspopup="true"
                  >
                    {/* Avatar */}
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#B5E18B] to-[#6aab2e] flex items-center justify-center text-[#1F2A44] font-black text-sm shadow-lg shadow-[#B5E18B]/20">
                      {getUserInitials()}
                    </div>
                    <span className="hidden sm:inline text-[#E0E8F0] text-sm font-semibold">
                      {session.user?.name?.split(" ")[0]}
                    </span>
                    <svg
                      className={`w-3.5 h-3.5 text-[#4a5568] transition-transform duration-200 ${dropdownOpen ? "rotate-180" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  {dropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-48 bg-[#0d1117] border border-white/[0.08] rounded-2xl shadow-2xl shadow-black/60 py-1.5 z-[200] overflow-hidden">
                      ...
                      <div className="px-4 py-2.5 border-b border-white/[0.06]">
                        <p className="text-[11px] font-black text-[#e2e8f0] uppercase tracking-wider truncate">
                          {session.user?.name}
                        </p>
              ...
              <button
                ...
                className="md:hidden w-9 h-9 rounded-xl bg-white/5 border border-white/[0.06] flex items-center justify-center text-[#8090A4] hover:text-white hover:bg-white/10 transition-all duration-200"
          ...
          <div className="relative h-[1.5px] bg-[#0d1422] overflow-hidden">
            <div
              className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-[#B5E18B]/70 to-transparent"
              style={{ animation: "bar-sweep 3.5s ease-in-out infinite" }}
            />
          </div>
        </div>
            ...
            <nav className="max-w-7xl mx-auto px-4 py-3 flex flex-col gap-0.5">
              {navLinks.map((l) => (
                <Link
                  key={l.href}
                  href={l.href}
                  className={`px-4 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-[0.2em] transition-colors duration-200 ${
                    (l.href === "/" ? pathname === "/" : pathname.startsWith(l.href))
                      ? "bg-[#B5E18B]/10 text-[#B5E18B]"
                      : "text-[#8090A4] hover:text-[#E0E8F0] hover:bg-white/5"
                  }`}
                >
                  {l.label}
                </Link>
        ...
        <div className="hidden sm:flex h-7 bg-[#0a0d14] border-b border-white/[0.04] items-center overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden relative">
            {tickerLoading ? (
              ...
              <span className="px-4 text-[#4a5568] text-[10px] uppercase tracking-wider font-semibold">
                No recent matches
              </span>
            ) : (
